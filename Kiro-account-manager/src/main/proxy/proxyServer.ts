...
export class ProxyServer {
  ...
  private handleAdminAccountsDetail(res: http.ServerResponse): void {
    ...
    const accounts: AccountDetailRow[] = storeRows.map(s => {
      ...
      return {
        ...
        // 配额耗尽态必须透出：402/429 时 recordError 只写 quotaExhaustedAt + errorCount，
        // **不动 isAvailable** —— 只看 isAvailable 的外部看板会在账号池被打爆时读绿。
  ...
  private async handleOpenAIChat(req: http.IncomingMessage, res: http.ServerResponse, signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      this.emitResponse({ path: '/v1/chat/completions', model: request.model, status: 400, error: message })
    ...
    if (!account) {
      ...
      this.emitResponse({ path: '/v1/chat/completions', model: request.model, status: 503, error: errorMsg })
  ...
  private async handleOpenAIResponses(req: http.IncomingMessage, res: http.ServerResponse, signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Invalid request'
      this.sendError(res, 400, message)
      this.emitResponse({ path: '/v1/responses', status: 400, error: message })
    ...
    if (!account) {
      ...
      const errorMsg = quotaStatus.exhausted > 0 && quotaStatus.available === 0
        ...
        : 'No available accounts'
      this.sendError(res, 503, errorMsg)
      this.emitResponse({ path: '/v1/responses', model: chatRequest.model, status: 503, error: errorMsg })
    ...
    return new Promise((resolve) => {
      this.callKiroApiStreamWithRetry(
        ...
        async (usage) => {
          ...
          this.emitResponse({ path: '/v1/chat/completions', model, status: 200, tokens: usage.inputTokens + usage.outputTokens, inputTokens: usage.inputTokens, outputTokens: usage.outputTokens, cacheReadTokens: usage.cacheReadTokens, cacheWriteTokens: usage.cacheWriteTokens, reasoningTokens: usage.reasoningTokens, credits: usage.credits, responseTime: oaiRespTime, accountId: account.id })
  ...
  private async handleClaudeMessages(req: http.IncomingMessage, res: http.ServerResponse, signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      this.emitResponse({ path: '/v1/messages', model: request.model, status: 400, error: message })
    ...
    if (!account) {
      ...
      this.emitResponse({ path: '/v1/messages', model: request.model, status: 503, error: errorMsg })
    ...
    return new Promise((resolve) => {
      this.callKiroApiStreamWithRetry(
        ...
        async (usage) => {
          ...
          this.stats.reasoningTokens += usage.reasoningTokens || 0
          const respTime = Date.now() - startTime
          // cacheWrite 的兜底与上面 stats.cacheWriteTokens 那行严格同构：真实值优先，缺失时用模拟器的 creation
          this.emitResponse({ path: '/v1/messages', model, status: 200, tokens: usage.inputTokens + usage.outputTokens, inputTokens: usage.inputTokens, outputTokens: usage.outputTokens, cacheReadTokens: usage.cacheReadTokens || simulatedCacheUsage?.cacheReadInputTokens, cacheWriteTokens: usage.cacheWriteTokens || simulatedCacheUsage?.cacheCreationInputTokens, reasoningTokens: usage.reasoningTokens, credits: usage.credits, responseTime: respTime, accountId: account.id })
